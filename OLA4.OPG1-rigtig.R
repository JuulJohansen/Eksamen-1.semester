...
#Rens DE
#Pris
calldf_de$price <- gsub("[^0-9.,]", "", calldf_de$price)
calldf_de$price <- gsub("\\.", "", calldf_de$price)
calldf_de$price <- as.numeric(calldf_de$price)
calldf_de$price_dk <- calldf_de$price*7.46
calldf_de <- calldf_de[,-1]
colnames(calldf_de)[which(colnames(calldf_de) == "price_dk")] <- "price"
calldf_de$price <- floor(as.numeric(calldf_de$price))
...
#Kilometer
calldf_de$Kilometer <- gsub("[^0-9.,]", "", calldf_de$Kilometer)
calldf_de$Kilometer <- gsub("\\.", "", calldf_de$Kilometer)  # Fjern punktum som tusindseparator
calldf_de$Kilometer <- as.numeric(calldf_de$Kilometer)

calldf_de$carid <- as.numeric(calldf_de$carid)
...
Alle_biler1$model <- ifelse(grepl("Single Motor", Alle_biler1$description, ignore.case = TRUE), 
                         paste(Alle_biler1$model, "Single Motor"), 
                         ifelse(grepl("Dual Motor", Alle_biler1$description, ignore.case = TRUE), 
                                paste(Alle_biler1$model, "Dual Motor"), 
                                Alle_biler1$model))
...
result_biler <- filtered_biler[filtered_biler$Year == "2022" & 
                                 filtered_biler$Kilometer >= 20000 & 
                                 filtered_biler$Kilometer <= 60000, ]

Sammenligning <- result_biler %>%
  group_by(Fra_Land) %>%
  summarise(Gennemsnit_Price = mean(price, na.rm = TRUE))


library(dplyr)

write.csv(result_biler, "Sammenligning_Biler.csv", row.names = FALSE)
